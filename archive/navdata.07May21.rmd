...
#make a table graph - don't remember what this does
x <- as_tbl_graph(net.tidy.1)
# layout, graphopt,kk
ggraph(net.tidy.1, layout = "kk") + 
  geom_node_point() +
  #geom_edge_link(width = 1, colour = "lightgray") +
  geom_node_point(size = 4, colour = "#00AFBB") +
  #geom_edge_link(aes(width = weight * 0.5), alpha = 0.8) + 
  geom_edge_link(aes(colour = "UnClassified Intranet"), method = "UnClassified Intranet", se = FALSE) + 
  geom_edge_link(aes(colour = "Classified Intranet"), method = "UnClassified Intranet", se = FALSE) + 
  scale_edge_width(range = c(0.2, 2)) +
  geom_node_text(aes(label = name), repel = TRUE) +
  labs(edge_width = "logical network type") +
  labs(colour ="Method") + scale_colour_manual(values = c("	
    Classified Intranet" = "magenta", "UnClassified Intranet
    " = "blue"))
  theme_graph()
...
text
```{r geom_edge_link.1, echo=TRUE}
# https://rdrr.io/cran/ggraph/man/geom_edge_link.html
# example of using geom_edge_link
library(ggraph)

require(tidygraph)
gr <- create_notable('Coxeter') %>%
  mutate(class = sample(letters[1:3], n(), replace = TRUE)) %>%
  # check out these pipes sometimes , he %N>% and %E>% pipes
  activate(edges) %>%
  mutate(class = sample(letters[1:3], n(), replace = TRUE))
...
gr_activate <- create_complete(5) %>%
  ...
  arrange(from)
# see the data frame
df.gr_activate <- as.data.frame(gr_activate)
graph(gr_activate, 'auto') +
  geom_edge_link(aes(alpha = stat(index)))

ggraph(gr_activate, 'auto') +
  geom_edge_link2(aes(colour = node.class))
