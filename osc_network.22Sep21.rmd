...
# https://www.webpages.uidaho.edu/~stevel/504/igraph.pdf
#options(scipen=999)
...
```{r graph1, eval=FALSE, include=FALSE}
...
net.igraph.1 <- graph_from_data_frame(df.igraph.1, directed = TRUE)
...
```{r tkplots, eval=FALSE, include=FALSE}
#tkplot(net.igraph.1)
plot(net.igraph.1, edge.arrow.size = 0.04,
...
# apparently the factor(Transport) is picked up from the data frame, nothing in igraph list
df.igraph.1 <- read.csv("node_asis.csv", blank.lines.skip = TRUE, strip.white = TRUE)
...
text
```{r scholastic, eval=FALSE, include=FALSE}
...
```{r geom_edge_link.2, eval=FALSE, include=FALSE}
